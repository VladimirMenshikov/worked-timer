...
Результат: `output\WorkTimerSetup-1.0.0.exe`.

## После установки

Настройка та же, что и для остальных версий — общая база в Supabase:

1. Рядом с `WorkTimer.exe` переименуйте `.env.example` в `.env` и заполните `SUPABASE_URL` / `SUPABASE_KEY`.
2. Таблица `wh_work_log` уже должна существовать в Supabase (см. [`../../sql/migrations`](../../sql/migrations)) — если нет, выполните миграции по порядку.
3. Запустите `WorkTimer.exe` — в трее появится зелёный кружок.
